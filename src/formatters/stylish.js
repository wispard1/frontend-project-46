const stylish = (obj1, obj2, depth = 0) => {
  ...
  const formatValue = (value, currentDepth) => {
    ...
    if (typeof value === 'object' && value !== null) {
      ...
      const nestedDiff = nestedKeys.map((key) => {
        ...
        return `${' '.repeat((currentDepth + 1) * space + 4)}${key}: ${nestedValue}`;
      });
      return `{\n${nestedDiff.join('\n')}\n${' '.repeat((currentDepth + 1) * space)}}`;
    }
    return String(value);
  };
  ...
  const diff = keys.flatMap((key) => {
    ...
    if (typeof value1 === 'object' && value1 !== null && typeof value2 !== 'object' && value2 !== undefined) {
    ...
    if (typeof value2 === 'object' && value2 !== null && typeof value1 !== 'object' && value1 !== undefined) {
